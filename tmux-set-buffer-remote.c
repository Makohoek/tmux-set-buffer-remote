...
static int forge_tmux_command(const struct linked_list text, char** command)
{
    ...
    size_t tmux_command_size = sizeof(char) *
                               (strlen(tmux_buffer_prefix) +
                                strlen(set_buffer_postfix)) + linked_list_data_size(text) + 1;
    ...
    *command = strcat(*command, tmux_buffer_prefix);
    ...
    // add text to copy
    linked_list_for_each(text, node)
        *command = strncat(*command, node->buf, LIST_BUFSZ);
